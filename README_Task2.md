...
5. ОТ точки 5 до точки 6 хорошо видна работа "сборщика" (Помечено зеленым восклицательным знаком). Мне кажется, в этот момент "подрезается" размер Листа ( зарезервированные пустые ячейки памяти) т.к. обращения к нему больше нет. Другие периоды активити "сборщика" не видны явным образом на графиках, помечены красным крестом. По моему мнению, это связано с тем, что обращение к Листу продолжалось до точки 5. 
